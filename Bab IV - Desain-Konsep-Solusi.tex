...
mengimplementasikan setiap komponen modul \textit{Mission}.

\input{table/tabel4-1.tex}
...
secara langsung. Secara garis besar, terdapat 4 kelompok utama data yang 
menjadi target \textit{crawling}.

\input{table/tabel4-2.tex}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{image/flowchart-mission.png}
...
akan diproses. Modul kemudian memilih target berikutnya dari daftar, dan memeriksa apakah masih 
terdapat artefak yang harus dikumpulkan. Jika tidak ada target tersisa, \textit{batch crawling} 
dianggap selesai dan proses dapat berlanjut ke tahap berikutnya. Untuk setiap target yang tersedia, modul mengambil data melalui \textit{crawling engine}. 
...
Mekanisme di dalamnya mencakup proses \textit{packaging, encoding, compression,} enkripsi, pemilihan 
...
\begin{enumerate}
    ...
    penggabungan, kompresi, \textit{lightweight encryption}, serta \textit{encoding data}. Tujuannya adalah mengurangi ukuran \textit{payload}, 
...
Pada modul \textit{Exfiltration}, setiap komponen dirancang untuk mendukung proses 
pengiriman data secara tersembunyi (\textit{stealth}), selaras dengan pendekatan 
\textit{fileless} dan \textit{low profile traffic} yang digunakan dalam arsitektur 
spyware. Agar setiap fungsi dapat berjalan secara efisien dan tetap sulit 
terdeteksi oleh mekanisme \textit{anti-spyware}, pemilihan teknologi harus 
disesuaikan dengan karakteristik dan kebutuhan masing-masing komponen. 
Tabel~\ref{tbl:teknologi-modul-exfiltration} merangkum teknologi yang digunakan dalam 
implementasi seluruh komponen pada modul \textit{Exfiltration}.

\input{table/tabel4-3.tex}
...
Tabel~\ref{tbl:target-eksfiltrasi}.

\input{table/tabel4-4.tex}
...
\input{table/tabel4-6.tex}

Lingkungan pengujian pada penelitian ini dibangun menggunakan sistem operasi Windows 
11 yang dikonfigurasi secara terisolasi untuk memastikan bahwa proses \textit{crawling} 
dan \textit{eksfiltrasi} tidak berdampak pada sistem di luar area eksperimen. Dari sisi 
perangkat, mesin uji menggunakan spesifikasi Intel Core i5 Gen 8, RAM 8 GB, SSD 1 TB. 
Pengujian melibatkan dua jenis jaringan, yaitu jaringan publik (ITB) dan jaringan lokal. 
Untuk \textit{endpoint} \textit{eksfiltrasi}, digunakan \textit{REST API} berbasis \textit{FastAPI} 
yang berfungsi sebagai \textit{server} penerima data dan menjadi target proses pengiriman \textit{payload}. 
Solusi keamanan komersial di antaranya adalah Windows Defender, AVG, Avast, dan McAfee. 
Keempat antivirus ini berperan sebagai \textit{baseline} deteksi yang umum ditemukan pada 
sistem \textit{end user}.
...
Pada desain ini, \textit{data crawling} dan eksfiltrasi akan dilakukan dalam bentuk \textit{batch}. 
Sistem memulai proses dengan empat kelompok data utama, yaitu 
\textit{keylogging data}, \textit{system metadata}, \textit{system artifacts}, \textit{internet cache browsing}. 
...
Data yang berhasil dieksfiltrasi akan disimpan pada \textit{Exfiltration Endpoint}. 
Jika masih terdapat artefak yang belum diproses atau \textit{buffer} belum memenuhi kriteria 
pengiriman, sistem akan melanjutkan eksekusi ke \textit{batch} berikutnya hingga seluruh 
target \textit{crawling} terselesaikan. Setelah seluruh \textit{batch} berhasil dieksfiltrasi, 
data yang tersimpan pada \textit{endpoint} menjadi input bagi \textit{Big Data Analytic} untuk 
